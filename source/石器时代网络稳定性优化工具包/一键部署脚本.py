    ...
    def generate_report(self, validation_results):
        ...
        report = {
            ...
            "summary": {
                ...
                "failed_checks": sum(1 for v in validation_results["checks"].values() if not v),
...
def main():
    ...
    print(f"验证完成: {summary['passed_checks']}/{summary['total_checks']} 项通过")
    
    if summary["error_count"] > 0:
        print(f"发现 {summary['error_count']} 个错误:")
        for error in report["details"]["errors"]:
            print(f"  • {error}")
    
    if summary["warning_count"] > 0:
        print(f"发现 {summary['warning_count']} 个警告:")
